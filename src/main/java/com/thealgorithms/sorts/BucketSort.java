 ...
 * 桶排序,将一组数据分配到不同的桶内,因为是由hash完成的,桶与桶之间的最大数是递增的.
 * 再在桶内进行排序,最后合并,就能实现排序
 */
public class BucketSort implements SortAlgorithm {
   ...
   * be placed.
   * This is done by "normalizing" the element within the range of the array's
   ...
   */
  // 这个是重点,分桶时能够保证桶之间的大小是有规律的
  // 具体就是拿到整个数组的最大值和最小值,之后算出当前元素在整体中的位置(也就是0~1间的小数)
  // 之后那这个比例乘以桶的总数就能算出当前元素应该在的位置
